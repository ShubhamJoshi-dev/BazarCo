...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const wrap = <T,>(fn: (arg: T) => void): ((...args: unknown[]) => void) => (...args: unknown[]) => fn(args[0] as T);
    const messageW = wrap(onMessage);
    const unsentW = wrap(onUnsent);
    const statusW = wrap(onMessageStatus);
    const typingW = wrap(onTyping);
    const typingStopW = wrap(onTypingStop);
    const onlineW = wrap(onUserOnline);
    const offlineW = wrap(onUserOffline);
    on("message", messageW);
    on("message_unsent", unsentW);
    on("message_status", statusW);
    on("typing", typingW);
    on("typing_stop", typingStopW);
    on("user_online", onlineW);
    on("user_offline", offlineW);
    return () => {
      off("message", messageW);
      off("message_unsent", unsentW);
      off("message_status", statusW);
      off("typing", typingW);
      off("typing_stop", typingStopW);
      off("user_online", onlineW);
      off("user_offline", offlineW);
      if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
    };
