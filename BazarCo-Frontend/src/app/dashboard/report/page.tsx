"use client";

import Link from "next/link";
import { BarChart3 } from "lucide-react";
import { useTranslations } from "next-intl";
import { useAuth } from "@/contexts/AuthContext";
import { SellerReportsView } from "@/components/dashboard/SellerReportsView";

export default function ReportPage() {
  const { user, loading } = useAuth();
  const t = useTranslations("sellerReports");

  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[40vh] text-neutral-500 text-sm">
        {t("loading")}
      </div>
    );
  }

  if (user?.role === "seller") {
    return <SellerReportsView />;
  }

  return (
    <div className="rounded-2xl border border-[var(--brand-border)] bg-[var(--card-bg)] p-12 text-center max-w-lg mx-auto">
      <BarChart3 className="mx-auto w-12 h-12 text-neutral-400 mb-4" />
      <h1 className="text-lg font-bold text-[var(--foreground)]">{t("sellerOnlyTitle")}</h1>
      <p className="text-sm text-neutral-500 mt-2">{t("sellerOnlyDesc")}</p>
      <Link
        href="/dashboard"
        className="inline-block mt-6 text-sm font-medium text-[var(--brand-blue)] hover:underline"
      >
        {t("backToDashboard")}
      </Link>
    </div>
  );
}
